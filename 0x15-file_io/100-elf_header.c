...
void print_data(unsigned char *e_ident)
{
	...
	{
		case ELFDATANONE:
OA			printf("Unknown data format\n");
OA			break;
OA		case ELFDATA2LSB:
OA			printf("2's complement, little endian\n");
OAOA			break;
		case ELFDATA2MSB:
...
OAOA{
OAOA	printf("  Version:                           ");
OAOAOA	if (e_ident[EI_VERSION] == EV_CURRENT)
		printf("%i (current)\n", EV_CURRENT);
	else
		printf("%i\n", e_ident[EI_VERSION]);
}
...
OA	switch (e_ident[EI_OSABI])
	{
OAOAOAOA		case ELFOSABI_SYSV:
...
OAOAOAOAOA			printf("UNIX - FreeBSD\n");
OAOA			break;
...
OAOA{
OA	if (e_ident[EI_DATA] == ELFDATA2MSB)
OA		e_type = e_type >> 8;
OA
	printf("  Type:                              ");
	switch (e_type)
	{
		...
		case ET_EXEC:
			...
			break;
		case ET_DYN:
OA			printf("DYN (Shared object file)\n");
OAOA			break;
OA		case ET_CORE:
 ...
 * Return: output uint
 */
OAunsigned int lit_to_big_endian(unsigned int x)
OA{
